...
const Stats = () => {
  ...
  return (
    <section className="relative py-16 md:py-24 bg-primary overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-64 h-64 bg-accent/10 rounded-full -translate-x-1/2 -translate-y-1/2 blur-3xl pointer-events-none" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-20">
          {stats.map((stat: any, idx: number) => (
            <div key={idx} className="text-center group">
              ...
              <div className="text-base md:text-lg text-accent-light font-bold tracking-wide uppercase opacity-80 group-hover:opacity-100 transition-opacity">
                {stat.label}
              </div>
              <div className="w-10 h-1 bg-accent/30 mx-auto mt-4 md:mt-6 rounded-full group-hover:w-16 group-hover:bg-accent transition-all duration-500" />
            </div>
          ))}
        </div>
      </div>
